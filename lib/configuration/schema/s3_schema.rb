...
module Schema
  class S3Schema < Dry::Validation::Contract
    ...
    rule(:s3) do
      key(%i[s3 endpoint]).failure('endpoint must be a valid url (e.g. https://s3.eu-west-1.amazonaws.com)') unless value && %r{https?://(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()!@:%_+.~#?&/=]*)}.match?(value[:endpoint])
    end
  end
end
