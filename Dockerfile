...
RUN apk update && apk add --no-cache postgresql-client postgresql && apk add --no-cache build-base libxml2-dev libxslt-dev tzdata && apk add ca-certificates curl && apk add --no-cache ruby-full && apk add --no-cache build-base libpq git
